...
public class CustomerServiceImpl implements CustomerService {
    ...
    @Override
    public Object customerDetails(Map<String, Object> map) throws ParseException {
        long page = 1;
        long limit = 10;
        ...
        patientIds.forEach(patientId -> {
            ...
            customerVo.put("patientPhone", patientUserMap.get(patientId).getMobile());
            // 循环拼接分页的数据
            // 获取聊天客服与当前患者所有的聊天消息
            ...
            customerVo.put("startTime", format.format(start * 1000));
            //当前客服聊天结束时间
            Long end = collect.stream().sorted(Comparator.reverseOrder()).collect(Collectors.toList()).get(0);
            customerVo.put("endTime", format.format(end * 1000));
            // 客服id
            customerVo.put("adminId", map.get("adminId").toString());
            // 患者id
            customerVo.put("patientId", patientId);
            list.add(customerVo);
        });
            ...
            if (org.apache.commons.lang3.StringUtils.isNotBlank(teamId)) {
                ...
                if (doctorUsers.size() == 1) {
                    ...
                    for (SearchTeamDoctorVo doctorUser : doctorUsers) {
                         ...
                         *  is_cancel = '0'
                         */
//                        if (!"1".equals(doctorUser.getIsMdt())||!"PASS".equals(doctorUser.getIsAuth()) || !"1".equals(doctorUser.getStatus()) || !"0".equals(doctorUser.getIsCancel())) {
//                            isNormal = false;
//                            break;
//                        }
                    }
    ...
    public JSONArray mdtList() {
        ...
        for (Mdt mdt : mdts) {
            ...
            jsonObject.put("mdtId", String.valueOf(mdt.getId()));
            jsonObject.put("mdtName", String.valueOf(mdt.getName()));
