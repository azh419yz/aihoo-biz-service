...
import com.aihoo.api.doctor.common.utils.TimeUtil;
...
//            logService.setVisitLog(hosVisit, StatusEnumUtil.START, logService.getDoctorUserName() + "医生开始与患者" + logService.getSickName(hosVisit.getHosSickId()) + "的专家咨询聊天。");
...
//            resultMap.put("msg", "开始专家咨询");
            resultMap.put("is_succ", "success");
//            imService.sendImSystemMsg(PATIENT + hosVisit.getPatientUserId(), DOCTOR + hosVisit.getDoctorUserId(), "问诊已开始，本次问诊可持续24小时", IMMsgType.PToDFirstText, "问诊已开始，本次问诊可持续24小时", "", "");
//            imService.sendImSystemMsg(PATIENT + hosVisit.getPatientUserId(), DOCTOR + hosVisit.getDoctorUserId(), "温馨提示：患者购买了您的图文问诊，请在24小时内回复患者，如在图文问诊结束前，您未回复患者，患者可要求全额退款。", IMMsgType.PToDSecondText, "温馨提示：患者购买了您的图文问诊，请在24小时内回复患者，如在图文问诊结束前，您未回复患者，患者可要求全额退款。", "", "");
            Map<String, Object> map = new HashMap<>();
            map.put("msgType", IMMsgType.DToPFirstText);
            map.put("msg", "医生已接单，您可以进行提问");
            map.put("businessID", "MSHCustomMsg");
//            imService.sendPostHttpRequest(DOCTOR + hosVisit.getDoctorUserId(), PATIENT + hosVisit.getPatientUserId(), map, "医生已接单，您可以进行提问", "", "");
            sendStartVisitIm(hosVisit.getOrderNum());
        } else if (status.equals(StatusEnumUtil.START)) {
            resultMap.put("msg", "在线复诊已开始");
//            resultMap.put("msg", "专家咨询已开始");
            resultMap.put("is_succ", "success");
        }
...
//        imService.sendImSystemMsg(PATIENT + hosVisit.getPatientUserId(), DOCTOR + doctorUser.getPapersNumbers(), "专家咨询已开始，本次专家咨询可持续24小时", IMMsgType.PToDFirstText, "专家咨询已开始，本次专家咨询可持续24小时", "", "");
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
//        imService.sendImSystemMsg(PATIENT + hosVisit.getPatientUserId(), DOCTOR + doctorUser.getPapersNumbers(), "温馨提示：患者购买了您的专家咨询，请在24小时内回复患者，如在专家咨询结束前，您未回复患者，患者可要求全额退款。", IMMsgType.PToDSecondText, "温馨提示：患者购买了您的专家咨询，请在24小时内回复患者，如在专家咨询结束前，您未回复患者，患者可要求全额退款。", "", "");
...
//                log.error("服务提醒：进入诊室，专家咨询通道建立，没有查到医生信息");
                return true;
            }
...
//                imService.sendPostHttpRequest(PATIENT + hosRevisit.getPatientUserId(), DOCTOR + hosRevisit.getDoctorUserId(), hosRevisit, desc, "", "",2);
                /********本来这里是接单发的，现在进入诊室再发**********/
//                imService.sendImSystemMsg(PATIENT + hosRevisit.getPatientUserId(), DOCTOR + hosRevisit.getDoctorUserId(), "复诊已开始，本次复诊可持续24小时", IMMsgType.PToDFirstText, "复诊已开始，本次复诊可持续24小时", "", "",1);
//                imService.sendImSystemMsg(PATIENT + hosRevisit.getPatientUserId(), DOCTOR + hosRevisit.getDoctorUserId(), "温馨提示：系统分配了一例复诊，请在24小时内回复患者，如在复诊结束前，您未回复患者，患者可要求全额退款。", IMMsgType.PToDSecondText, "温馨提示：系统分配了一例复诊，请在24小时内回复患者，如在复诊结束前，您未回复患者，患者可要求全额退款。", "", "",2);
                Map<String, Object> map = new HashMap<>();
                map.put("msgType", IMMsgType.DToPFirstText);
                map.put("msg", "医生已接单，您可以进行提问");
                map.put("businessID", "MSHCustomMsg");
//                imService.sendPostHttpRequest(DOCTOR + hosRevisit.getDoctorUserId(), PATIENT + hosRevisit.getPatientUserId(), map, "医生已接单，您可以进行提问", "", "",3);
                resultMap.put("isCanChat", "1");
...
//            logService.setVisitLog(hosVisit, StatusEnumUtil.END, logService.getDoctorUserName() + "医生结束与患者" + logService.getSickName(hosVisit.getHosSickId()) + "的专家咨询聊天。");
            HosVisit updHosVisit = new HosVisit();
            updHosVisit.setId(id);
...
//            boolean sendImSystemMsg = imService.sendImSystemMsg(PATIENT + hosVisit.getPatientUserId(), DOCTOR + doctorIdCard, "本次咨询已结束", IMMsgType.EndText, "本次咨询已结束", "", "");
            if (sendImSystemMsg) {
                imService.sendPostHttpRequest(DOCTOR + hosVisit.getDoctorUserId(), PATIENT + hosVisit.getPatientUserId(), mapIm, "在线复诊评价", "", "");
//                imService.sendPostHttpRequest(DOCTOR + doctorIdCard, PATIENT + hosVisit.getPatientUserId(), mapIm, "专家咨询评价", "", "");
            }
        }
        return resultMap;
    }
...
//            boolean sendImSystemMsg = imService.sendImSystemMsg(PATIENT + hosRevisit.getPatientUserId(), DOCTOR + doctorIdCard, "本次咨询已结束", IMMsgType.EndText, "本次咨询已结束", "", "");
            if (sendImSystemMsg) {
...
//        pushMessageQueryWrapper.eq("is_delete","0").eq("notice_type",noticeType).;
